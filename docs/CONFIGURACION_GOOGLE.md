...
1. Crear la aplicacion con el paquete definitivo `com.gemidospremium.app`. Este identificador no puede cambiar despues de publicar.
2. Crear un producto integrado no consumible con ID exacto `premium_silence_pack`.
...
La app no muestra ni guarda formularios de tarjeta. `BillingClient.launchBillingFlow` abre la pantalla administrada por Google Play. Antes de un lanzamiento a escala relevante conviene validar el token de compra en un backend mediante Google Play Developer API; queda registrado en el backlog.
...
```properties
GEMIDOSPREMIUM_ADMOB_APP_ID=ca-app-pub-XXXXXXXXXXXXXXXX~YYYYYYYYYY
GEMIDOSPREMIUM_ADMOB_BANNER_ID=ca-app-pub-XXXXXXXXXXXXXXXX/ZZZZZZZZZZ
```
...
- conservar siempre `Apagado plebeyo` como corte gratuito, visible e inmediato;
- completar la clasificación de contenido y declarar de forma precisa el audio vocal sugerente.
