...
- La celebracion Premium reproduce una mezcla Ogg Vorbis de 15 segundos: conserva cinco fanfarrias/aplausos/vitores originales y suma los siete efectos Mixkit aprobados (candidatos 2 a 8, nunca 1/462), escalonados en cinco tandas con un solo premio/casino y como maximo dos efectos humanos nuevos simultaneos. Deja cuatro descansos reales, no tiene base constante y se detiene al salir, reiniciar o restablecer Premium. Fuentes, hashes y tiempos viven en `tools/premium-celebration-plan.json`; los MP3 fuente no se versionan.
- Las herramientas raiz usan Node 24, alineado con CI. `npm test` incluye 12 pruebas del plan sonoro con umbral 100 % de lineas/ramas/funciones; `npm run test:audio` verifica localmente el OGG mediante FFmpeg, sin agregar descargas ni codecs al CI rapido.
