...
test("Los siete sonidos aprobados aparecen en cinco tandas con descansos", () => {
...
rejects("No superpone dos sonidos de casino", (value) => { value.events.push({ ...value.events[2], startMs: 220 }); }, /Nunca dos premios/);
rejects("No amontona todas las personas", (value) => { value.events.push({ ...value.events[3], startMs: 6350 }); }, /maximo dos/);
rejects("No extiende el audio mas alla de los 15 segundos", (value) => { value.events[0].startMs = 14900; }, /extender/);
rejects("Mantiene los silencios entre tandas", (value) => { value.events[1].durationMs = 2500; }, /descansos/);
rejects("No lee fuera del clip fuente", (value) => { value.events[5].offsetMs = 500; }, /fuente/);
...
rejects("Los descansos no pueden desaparecer", (value) => { value.bursts[1].startMs = 2400; }, /descanso/);
